...
let Action;
let inputName = player.userName;
let playerName = inputName.charAt(0).toUpperCase() + inputName.slice(1);
let playerHp = Math.floor(Math.random() * (150 - 40) + 40);
let opponentHp = Math.floor(Math.random() * (150 - 40) + 40);
let newoppoHp = opponentHp;
let newplayHp = 0; //here we initialize it with a default value "0" bcuz In TypeScript, variables must be assigned a value before they are used. This rule is in place to prevent potential runtime errors that could occur if you try to use a variable that hasn't been initialized with a value.
let isAttack = false;
let isEnergy = false;
;
let energyDrink = 3;
let afterEnergyDose = playerHp;
let afterEnergyDoseNotAttack = playerHp;
